...
## Testing the Coding Agent with tmux

To test the coding agent TUI in a controlled terminal environment:

```bash
# Create tmux session with specific dimensions
tmux new-session -d -s coding-agent-test -x 80 -y 24

# Start the coding agent from source
tmux send-keys -t coding-agent-test "cd /path/to/Bindery && ./pi-test.sh" Enter

# Wait for startup, then capture output
sleep 3 && tmux capture-pane -t coding-agent-test -p

# Send input
tmux send-keys -t coding-agent-test "your prompt here" Enter

# Send special keys
tmux send-keys -t coding-agent-test Escape
tmux send-keys -t coding-agent-test C-o  # ctrl+o

# Cleanup
tmux kill-session -t coding-agent-test
```
