...
Pi bundles a small set of runtime helpers for extensions and skills. If you import any of these, declare them in `peerDependencies` with a "*" range and do not bundle them: `@earendil-works/pi-ai`, `@earendil-works/pi-agent-core`, `@earendil-works/pi-coding-agent`, `typebox`.
